...
**Low-latency / gaming tweaks** (esports profile, SAFE + MODERATE):
`NetworkThrottlingIndex` off, Nagle off + TCP autotune,
...
.NET Desktop Runtime 8 · Edge WebView2 · DirectX June 2010 runtime ·
**Runtime optimizer** · **Roblox** · **Roblox Account Manager**.

**Runtime optimizer** — a small Native-AOT background service (no .NET runtime)
installed to `C:\ProgramData\GameRuntime` and registered as a highest-privilege
**logon task** that runs hidden. It manages running Roblox instances (per-instance
CPU/memory/priority tuning). It replaces Mem Reduct here; set
`InstallMemReduct = $true` in `$Config` if you also want the generic RAM cleaner.
...
- **Reversible** — before touching anything it exports the registry subtrees it
  ...
  matching `.reg`, or roll back the whole thing in one command (run elevated):
  ```powershell
  $env:DEPLOY_ROLLBACK='1'; irm https://raw.githubusercontent.com/xelasleepi/vps/main/install.ps1 | iex
  ```
  That removes the runtime optimizer + startup tasks, re-enables the disabled
  services, re-imports the newest registry backup, and clears the checkpoint.
- **Integrity-checked exe** — the elevated runtime exe is verified against a
...
- **Compatible optimizer** — the tweaks deliberately leave Windows Power
  Throttling **on** so the runtime optimizer's EcoQoS can still park idle Roblox
  instances.
- **Health check** — a final pass verifies the real end-state (runtime service
  running, `VPS-Opti`/`GameRuntime` tasks registered, Roblox/WinRAR present, key
  tweaks actually stuck) and reports `Health: X/Y checks passed` — so you know it
  *worked*, not just that it *ran*.
