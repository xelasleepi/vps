...
# ============================================================================
# Resilient by default. Native tools (taskkill, powercfg, sc, reg, fsutil…)
# routinely write to stderr / return non-zero, and with EAP=Stop that becomes a
# FATAL terminating error — confirmed in Windows PowerShell 5.1, which is what
# Tiny10 elevates into. So use Continue globally and rely on explicit
# `-ErrorAction Stop` inside the specific try/catch blocks that need it.
$ErrorActionPreference = 'Continue'
$PSNativeCommandUseErrorActionPreference = $false   # belt-and-suspenders for PS 7.4+
...
if (-not $isAdmin) {
    ...
    try {
        Start-Process -FilePath $psExe -Verb RunAs -ArgumentList @(
            ...
            '-Command', "`$env:DEPLOY_ROLLBACK='$($env:DEPLOY_ROLLBACK)'; `$env:DEPLOY_RESET='$($env:DEPLOY_RESET)'; irm $SelfUrl | iex"
        )
    } catch {
...
$Config = [ordered]@{
    ...
    Features = [ordered]@{
        ...
        InstallDirectX             = $true
        InstallRuntime             = $true   # background runtime optimizer (supersedes Mem Reduct)
        InstallMemReduct           = $false  # replaced by the runtime optimizer; flip to $true to run both
...
function Invoke-Optimizations {
    ...
    # ---- curated low-latency / gaming tweaks (esports profile, SAFE+MODERATE) ----
    Do-Opt 'netlatency' 'Latency & responsiveness (throttling off, Nagle off, timer res)' {
        ...
        Set-Reg $sp 'SystemResponsiveness' 10                              # 10 (not 0 — avoids audio/net starvation)
    ...
    Do-Opt 'cpumem' 'CPU & memory (core unpark, boost, caches off)' {
        # NOTE: Power Throttling is deliberately left ENABLED. The runtime optimizer
        # uses EcoQoS (POWER_THROTTLING_EXECUTION_SPEED) to park idle Roblox instances
        # on E-cores; a global PowerThrottlingOff=1 would defeat that. Do not re-add.
...
REM (Power Throttling left ON on purpose — the runtime optimizer's EcoQoS needs it.)
...
function Invoke-Software {
    ...
    # Background runtime optimizer (Native-AOT console).
    if ($f.InstallRuntime) { Install-Runtime } else { Write-Skip 'Runtime optimizer' '(disabled in config)' }
...
function Install-Runtime {
    # Downloads the background runtime optimizer (Native-AOT, no .NET runtime) to a
    # no-space path, registers a highest-privilege logon task so it runs hidden in
    # the background at every startup (needs admin for standby-list flush), and
    # launches it now.
    $sw  = [Diagnostics.Stopwatch]::StartNew()
    $dir = Join-Path $env:ProgramData 'GameRuntime'    # C:\ProgramData\GameRuntime — no spaces (schtasks-friendly)
    $exe = Join-Path $dir 'GameRuntime.exe'
    $url = 'https://github.com/xelasleepi/vps/releases/download/runtime/GameRuntime.exe'
    $label = 'Runtime optimizer'

    if ((Test-Path $exe) -and (Test-Done 'sw.Runtime')) {
        Write-Skip $label '(done in a previous run)'; Record $label 'Skipped' 0 'present'; return
    }

    # Fetch the expected SHA-256 from the release sidecar so the elevated exe is
    # integrity-verified. Use WebClient.DownloadString: in Windows PowerShell 5.1
    # Invoke-WebRequest returns a release asset's .Content as a Byte[], which
    # would parse to garbage and cause a false hash mismatch.
    $expected = $null
    try {
        $wc = New-Object System.Net.WebClient
        $expected = (($wc.DownloadString("$url.sha256")) -split '\s+')[0].Trim()
    } catch { }

    $null = New-Item -ItemType Directory -Force -Path $dir
    Get-Process GameRuntime -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue  # stop a running copy so we can overwrite
    if (-not (Get-File -Url $url -Dest $exe -Label $label -Sha256 $expected)) {
        Write-Fail $label 'download failed'; Set-Step 'sw.Runtime' 'Failed' 'download'; Record $label 'Failed' $sw.Elapsed.TotalSeconds; return
    }

    # Auto-start hidden at logon (a Task Scheduler console app has no visible window).
    schtasks /Create /TN 'GameRuntime' /TR $exe /SC ONLOGON /RL HIGHEST /F 2>$null | Out-Null
    Start-Process $exe -WindowStyle Hidden -ErrorAction SilentlyContinue

    if (Test-Path $exe) {
        Write-Ok $label ("(auto-starts at logon, {0:N1}s)" -f $sw.Elapsed.TotalSeconds)
        Set-Step 'sw.Runtime' 'Done' 'installed'; Record $label 'Installed' $sw.Elapsed.TotalSeconds
    } else {
        Write-Fail $label 'exe missing after download'; Set-Step 'sw.Runtime' 'Failed' 'verify'; Record $label 'Failed' $sw.Elapsed.TotalSeconds
    }
}
...
function Invoke-HealthCheck {
    ...
    if ($f.OptimizeWindows) {
        ...
        [void]$checks.Add(@{ N = 'Network throttling disabled';    Ok = (((Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile' -Name NetworkThrottlingIndex -ErrorAction SilentlyContinue).NetworkThrottlingIndex) -in 4294967295, -1) })
    }
    if ($f.InstallRuntime) {
        [void]$checks.Add(@{ N = 'Runtime service running'; Ok = [bool](Get-Process GameRuntime -ErrorAction SilentlyContinue) })
        [void]$checks.Add(@{ N = 'Runtime logon task';      Ok = (Test-SchTask 'GameRuntime') })
    }
...
# Undoes a deployment: removes the runtime optimizer + startup tasks, re-enables
# the disabled services, re-imports the newest registry backup, and clears the
# checkpoint. Triggered by $env:DEPLOY_ROLLBACK='1'.
function Invoke-Rollback {
    Write-Banner
    Write-Section 'Rollback — undoing the deployment'
    Get-State

    # 1. Runtime optimizer: stop, remove logon task + folder.
    Get-Process GameRuntime -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
    schtasks /Delete /TN 'GameRuntime' /F 2>$null | Out-Null
    Remove-Item (Join-Path $env:ProgramData 'GameRuntime') -Recurse -Force -ErrorAction SilentlyContinue
    Write-Ok 'Removed runtime optimizer + logon task'

    # 2. VPS-Opti startup task + script.
    schtasks /Delete /TN 'VPS-Opti' /F 2>$null | Out-Null
    Remove-Item (Join-Path $Root 'vps-opti.bat') -Force -ErrorAction SilentlyContinue
    Write-Ok 'Removed VPS-Opti startup task'

    # 3. Re-enable the services the run disabled (best-effort → Manual).
    $svcs = 'SysMain','WSearch','DoSvc','XblAuthManager','XblGameSave','XboxGipSvc','XboxNetApiSvc',
            'DiagTrack','dmwappushservice','WerSvc','Fax','RemoteRegistry','WMPNetworkSvc','MapsBroker'
    $re = 0
    foreach ($s in $svcs) {
        if (Get-Service -Name $s -ErrorAction SilentlyContinue) { Set-Service -Name $s -StartupType Manual -ErrorAction SilentlyContinue; $re++ }
    }
    Write-Ok 'Re-enabled disabled services' "($re set to Manual)"

    # 4. Re-import the newest registry backup set (restores the tweaked subtrees).
    $backupDir = Join-Path $Root 'backup'
    $latest = if (Test-Path $backupDir) { Get-ChildItem $backupDir -Filter '*.reg' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1 } else { $null }
    if ($latest) {
        $stamp = (($latest.Name -split '_')[0..1]) -join '_'
        $set = Get-ChildItem $backupDir -Filter "$stamp*.reg"
        $ok = 0
        foreach ($f in $set) { reg import $f.FullName 2>$null | Out-Null; if ($LASTEXITCODE -eq 0) { $ok++ } }
        Write-Ok 'Restored registry backup' "($ok/$($set.Count) subtrees from $stamp)"
    } else {
        Write-Skip 'Registry backup' '(none found — use a System Restore point if available)'
    }

    # 5. Clear the checkpoint so a future run redeploys from scratch.
    $hadRp = Test-Done 'safety.restorepoint'
    Remove-Item $StateFile -Force -ErrorAction SilentlyContinue
    ...
    Write-Host '  ╚════════════════════════════════════════════════════╝' -ForegroundColor Cyan
    Write-Host '   A reboot is recommended to fully apply.' -ForegroundColor Gray
    if ($hadRp) { Write-Host '   A System Restore point from before deployment also exists (run rstrui.exe).' -ForegroundColor DarkGray }
    Write-Host ''
    Write-LogFile '[ROLLBACK] deployment undone'
}
...
# ============================================================================
try {
    if ($env:DEPLOY_ROLLBACK -eq '1') {
        Invoke-Rollback       # undo a previous deployment, then stop
    } else {
        Write-Banner
